...
FETCH_WECHAT = Path(__file__).parent / "fetch-wechat.py"
SECTIONS = ("Pending", "Imported", "Failed")
TRAILING_URL_PUNCT = ".,;:!?，。；：！？)]）】"


def load_fetch_wechat():
    spec = importlib.util.spec_from_file_location("fetch_wechat_module", FETCH_WECHAT)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot load {FETCH_WECHAT}")
