...
- **Fixed pipeline, one fallback only.** The ONLY fallback is the LLM:
  ...
  silently.
- **No Telegram.** Owner reads locally. Digest -> `~/park-io/001_daily newsletter/ai/<date>.{md,html,png}`.
...
- **Post-proof source fix:** `克劳德猎手` WeWe RSS is now configured as
  `http://localhost:4000/feeds/MP_WXS_3935644082.json` in `~/park-io/_source management/sources.md`
...
- `PARKIO_BATCH_ID=20260604 python3 finalize-local.py` wrote
  `~/park-io/001_daily newsletter/ai/26-06-04.{md,html,png}`.
