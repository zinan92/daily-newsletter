...
python3 workflow/diagram/render-workflow-diagram.py \
  --input path/to/inbox-workflow.yaml \
  --html /tmp/inbox-workflow.html \
  --png /tmp/inbox-workflow.png \
  --json /tmp/inbox-workflow.json
```

The scripts live here because the reader-facing vault is an external runtime
artifact store. Keep Python utilities in this repo and pass explicit output
paths when rendering vault-specific views.
