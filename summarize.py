...
def event_title(event: dict) -> str:
    key = event["event_key"]
    if key.endswith(":xianyu-practice"):
        return display_title(event["primary"])
    if key == "openai-chatgpt-codex-mobile":
        return "ChatGPT / Codex 移动端代理预览"
    if event["primary"].get("source") == "openai-codex-releases":
        return display_title(event["primary"])
    if key == "openai-guaranteed-capacity":
        return "OpenAI Guaranteed Capacity：长期算力保障"
    if key == "openai-synthid-image-provenance":
        return "OpenAI SynthID 图像水印检测"
    if key == "codex-goal-command":
        return "Codex /goal 持久目标模式演示"
    if key == "anthropic-stainless-acquisition":
        return "Anthropic 收购 Stainless，强化 SDK 与工具链"
    if key in {"claude-code-quota-access", "anthropic-claude-code-toolchain-update"}:
        return "Claude Code 工具链与 Fast Mode 更新"
    if key == "claude-usage-credits":
        return "Claude 使用额度与 Fast Mode 计费规则更新"
    if key == "claude-prompt-cache-diagnostics":
        return "Claude Console 上线提示缓存诊断工具"
    if key == "claude-design-token-limits":
        return "Claude Design 推出，所有计划 token 限额翻倍"
    if key == "codex-enterprise-adoption":
        return "Codex 企业试用与采用信号升温"
    if key == "codex-windows-sandbox":
        return "Codex Windows 沙箱实现细节发布"
    if key == "opensquilla-agent-cost-router":
        return "OpenSquilla 用智能路由降低 Agent Token 成本"
    if key == "claude-small-business":
        return "Claude for Small Business 发布"
    items = event["items"]
    if len(items) == 1:
...
def display_title(item: dict) -> str:
    ...
    if item.get("source", "") in (source_names_for_group("twitter") | source_names_for_group("saved")):
        return title
    if item.get("source", "") in source_names_for_group("wechat"):
        return title
